...
| `tests/unit/test_ad_model.py` | 2026-08-23 | Die zwei Tests zur `sell_directly`-Regel prüfen jetzt Annahme statt Ablehnung, passend zur geänderten Regel. |
...
| `.github/workflows/update-python-deps.yml` | 2026-08-30 | Eine `repository_owner`-Bedingung am Job `update-python-deps` ergänzt. Der Job scheitert in jedem Fork täglich um 10 Uhr am Schritt „Generate GitHub Access Token", weil dieser die GitHub-App der Organisation Second-Hand-Friends verlangt; ein Fork hat deren Zugangsdaten nicht. Belegt an den Läufen vom 28., 29. und 30.08.2026. Abgeschaltet statt gelöscht, damit die Datei bei einem Upstream-Merge nicht kollidiert — dieselbe Lösung wie bei `build.yml`. |
